...
import { deletePago } from "../../../api/Pagos/deletePago";
...
export function ModalBodyDelete({ id }: { id: number }) {
  ...
  const handleDelete = async () => {
    if (!id) return;
    try {
      const response = await deletePago({
        id: id,
      ...
      if (response?.success) {
        ...
        setAnnouncement(
          ...
          <div className="flex items-center justify-center gap-4">
            ...
            <p className="font-medium text-white">Error al eliminar pago</p>
          </div>
        );
      }
    } catch (error) {
    ...
    <div className="flex flex-col h-full max-h-[50vh]">
      <div className="flex-1 pr-2">
        <p className="text-gray-800">
          ¿Está seguro de que desea eliminar al{" "}
          <span className="font-bold">pago</span> de ID:{" "}
