...
import { deleteEjercicio } from "../../../api/Ejercicios/deleteEjercicio";
import { useAnnouncement } from "../../../stores/Announcement/announcementStore";
import { FaCircleCheck } from "react-icons/fa6";
import { FaCircleXmark } from "react-icons/fa6";

export function ModalBodyDelete({
  id,
  nombre,
}: {
  id: string;
  ...
  const handleDelete = async () => {
    if (!id) return;
    try {
      const response = await deleteEjercicio({
        id: id,
    ...
    <div className="flex flex-col h-full max-h-[50vh]">
      <div className="flex-1 pr-2">
        <p className="text-gray-800">
          ¿Está seguro de que desea eliminar al ejercicio{" "}
          <span className="font-semibold text-red-600">{nombre}</span>?
        </p>
        <p className="mb-4">
