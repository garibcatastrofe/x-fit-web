...
export function ModalBodyFilter() {
  ...
  /* const renderDynamicInput = () => {
    if (
      ["nombres", "apellidos", "correo", "telefono"].includes(selectedAttribute)
    ) {
      return (
        <input
          type="text"
    ...
    if (["fecha_nacimiento", "fecha_inicio"].includes(selectedAttribute)) {
      return (
        <input
          type="date"
          className="w-full p-4 border-2 border-gray-100 outline-none rounded-xl"
    ...
    if (selectedAttribute === "genero") {
      return (
        <select className="w-full p-4 border-2 border-gray-100 outline-none rounded-xl">
          <option value="F">Femenino</option>
          <option value="M">Masculino</option>
        </select>
      );
    }
    if (selectedAttribute === "estatus") {
      return (
        <select className="w-full p-4 border-2 border-gray-100 outline-none rounded-xl">
    ...
    return null; // No muestra nada si no hay selección válida
  }; */
