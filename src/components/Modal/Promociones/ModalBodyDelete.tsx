...
import { deletePromocion } from "../../../api/Promociones/deletePromocion";
...
export function ModalBodyDelete({
  ...
  const handleDelete = async () => {
    if (!id) return;
    try {
      const response = await deletePromocion({
        id: id,
    ...
    <div className="flex flex-col h-full max-h-[50vh]">
      <div className="flex-1 pr-2">
        <p className="text-gray-800">
          ¿Está seguro de que desea eliminar la promoción de nombre: {nombre}?
        </p>
        <p className="mb-4">
