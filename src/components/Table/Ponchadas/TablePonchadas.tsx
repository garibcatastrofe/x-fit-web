...
/* FETCH */
import { selectAllPonchadas } from "../../../api/Ponchadas/selectAllPonchadas";
...
/* TYPES */
/* import { ConsultaPonchada } from "../../../types/Ponchadas/ConsultaPonchada"; */
import { ConsultaPonchadasUsuarios } from "../../../types/Ponchadas/ConsultaPonchadasUsuarios";
/* import { PonchadaPrimitive } from "../../../types/Ponchadas/PonchadaPrimitive"; */

/* FUNCTIONS */
import { formatDateTimeManual } from "../../../functions/date";

/* COMPONENTS */
import { ButtonCuadrado } from "../components/ButtonCuadrado";
import { PonchadasUsuarios } from "../../../types/Ponchadas/PonchadasUsuarios";
import { ModalBodyAdd } from "../../Modal/Ponchadas/ModalBodyAdd";
import { ModalBodyDelete } from "../../Modal/Ponchadas/ModalBodyDelete";
import { ModalBodyFilter } from "../../Modal/Ponchadas/ModalBodyFilter";

export function TablePonchadas({ columns }: { columns: string[] }) {
  const { setModalFilter, modalFilter } = useFilterModal();
  const [data, setData] = useState<ConsultaPonchadasUsuarios>();
  const [irSiguiente, setIrSiguiente] = useState(false);
  const { setMensaje, mensaje } = useMessageUpdated();
  const { setModal } = useModal();
  const navigate = useNavigate();

  const buscarPonchadas = async () => {
    const ponchadas: ConsultaPonchadasUsuarios = await selectAllPonchadas({
  ...
  const buscarSiguiente = async () => {
    const ponchadas = await selectAllPonchadas({
      buscarSiguiente: true,
    });
    if (ponchadas.data.length === 0) {
      ...
      if (accion == "EDITAR") {
        setModal(
          ...
          <>
            ...
            <p>Modal body actualizar, dato: {dato.ponchada.fecha}</p>
          </>
        );
      } else {
        setModal(true, "Eliminar ponchada", <ModalBodyDelete id={id} />);
      }
    } catch (error) {
      console.error("Error al obtener la ponchada", error);
    }
  };
    ...
    <div className="flex flex-col h-full">
      <div className="flex flex-col items-center justify-between gap-4 p-6 lg:flex-row md:flex-row">
        <div className="flex justify-center w-full gap-4 lg:justify-normal md:justify-normal">
          ...
          <ButtonCuadrado
            action={() =>
              setModal(true, "Filtrar ponchadas", <ModalBodyFilter />)
            }
            Icon={IoOptions}
            rotate={false}
            color="bg-red-600"
      ...
      <div className="flex-1 px-6 pb-6 overflow-y-auto scrollbar-custom">
        ...
        >
          ...
          {data?.count === 0 ? (
            <p>No se encontraron ponchadas</p>
          ) : (
                ...
                {data == null ? (
                  ...
                  data.data.map((dato, index) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-6 text-left whitespace-nowrap">
                        {formatDateTimeManual(dato.ponchada.fecha)}
                      </td>
                      <td className="px-3 py-6 text-left whitespace-nowrap">
